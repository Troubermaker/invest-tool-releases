...
// ---------------- 市场会话（A 股交易时段判定）----------------
// ⚠️ 跟后端 services/market_session.py 是同一份语义的两套实现。
//    8 档枚举名、时间边界（9:15 / 9:25 / 9:30 / 11:30 / 13:00 / 15:00）必须保持一致。
//    任一边修改时间边界都要同步改另一边，不然会发生「前端在刷但后端把缓存判为 offhours
//    24h TTL」这种死锁（集合竞价显示昨天行情的 bug 就是这么来的）。
//
// 'before-open'    0:00  - 9:15  盘前
// 'auction'        9:15  - 9:25  集合竞价（行情有变动 → 仍刷新）
// 'auction-quiet'  9:25  - 9:30  竞价静默期（撮合期，行情冻结 → 暂停 polling）
