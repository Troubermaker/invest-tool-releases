...
// adminOnly: true 表示仅管理员可见 —— 跟 Market/Watchlist 里扫描器入口的权限对齐
// 普通用户没法触发扫描，量化模块对他们没意义，没必要暴露入口
//
// 注意：原本的 candidates / quant / backtest / journal 4 个 tab 已并入「量化」一个 tab，
// 通过 QuantHub.vue 内部 sub-tab 切换；原 review 已并入「行情」，通过 MarketHub 切实时/复盘
const ALL_NAV_ITEMS = [
  { id: 'market', name: '行情', icon: 'M2.25 18 9 11.25l4.306 4.307a11.95 11.95 0 0 1 5.814-5.518l2.74-1.22m0 0-5.94-2.281m5.94 2.28-2.28 5.941' },
  ...
  // 量化中心：选股 / ⭐ 候选 / 回测 / 日志 4 个 sub-tab 整合（管理员专属）
  { id: 'quant', name: '量化', adminOnly: true, icon: 'M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 3m8.5-3l1 3m0 0l.5 1.5m-.5-1.5h-9.5m0 0l-.5 1.5M9 11.25v1.5M12 7.5v5.25m3-3v3' },
]

const navItems = computed(() =>
    ALL_NAV_ITEMS.filter(item => !item.adminOnly || isAdmin.value),
)
